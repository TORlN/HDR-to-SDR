...
class TestBatchQueue(unittest.TestCase):
    ...
    Only tests where the real _BatchMixin methods are mocked out (so
    they exercise gui.py's OWN dispatch logic -- handle_file_drop /
    select_file deciding whether to route through the queue -- not the
    mixin's real implementation) live here. Everything that calls the
    real add_batch_files/clear_batch_queue/remove_selected_batch_item/
    on_batch_item_select/_refresh_batch_list/_batch_item_for_current_input
    or reads _STATUS_ICONS moved to src/pro/test/batch_test.py's
    TestBatchQueue -- see task-6 of the Pro/private-repo split.
    """
